...
class GrupController {
  ...
  async subeEkle(req, res) {
    try {
      ...
      // Paket: grubun mevcut şubesinden inherit
      const ornek = (await pool.query('SELECT paket FROM isletmeler WHERE grup_id=$1 LIMIT 1', [grupId])).rows[0];
      ...
      const sube = (await pool.query(
        `INSERT INTO isletmeler (isim, slug, telefon, sehir, ilce, adres, kategori, aktif, paket, grup_id, sube_etiketi, olusturma_tarihi)
         VALUES ($1,$2,$3,$4,$5,$6,'genel',true,$7,$8,$9,NOW()) RETURNING *`,
